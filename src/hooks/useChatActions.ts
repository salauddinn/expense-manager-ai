...
export function useChatActions() {
  ...
  const resolveLinkedSource = useCallback(
    ...
    ): { linkedAccountId?: string; linkedCardId?: string } => {
  ...
  const resolveLoadingMessage = useCallback(
    async (
      ...
      setMessages((prev) => prev.map((m) => (m.id === loadingId ? { ...m, ...update } : m)));
  ...
  const handleSend = useCallback(
    (imageUrl?: string) => {
      ...
      if (isLLMConfigured) {
        const loadingMsg = createMessage('assistant', CHAT_LOADING_PLACEHOLDER, {
          isLoading: true,
        });
