...
// Authenticates via JWT, proxies to OpenAI or Google Gemini,
// keeps API keys server-side as Deno secrets.
...
async function callGemini(apiKey: string, model: string, message: string) {
  ...
  return { intent: fnCall.functionCall.name, data: args, message: args.message ?? 'Parsed.' };
}
